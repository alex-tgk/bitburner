...
export async function main(ns: NS): Promise<void> {
  ...
  const controllerThreads = calculateThreads(
    ns,
    'home',
    'services/port-controller-service.js',
  )
  ns.run(
    'services/port-controller-service.js',
    Math.floor(controllerThreads * 0.3),
  )

  // Start script that listens for incoming commands on port 1 and executes them.
  const listenerThreads = calculateThreads(
    ns,
    'home',
    'services/port-listen-service.js',
  )
  ns.run('services/port-listen-service.js', Math.floor(listenerThreads * 0.9))
}
