...
export async function main(ns: NS): Promise<void> {
    const ramOptions = [8, 16, 32, 64, 128, 256, 512, 1024, 2048, 4096, 8192, 16384, 32768]; // RAM options for servers, in GB
        ...
        for (const server of servers) {
            ...
            if (nextRamIndex < ramOptions.length) {
                ...
                if (currentFunds - upgradeCost > minimumBalance) {
                    ...
                    ns.tprint(`Upgraded server ${server.hostname} to ${newHostname} with ${nextRam}GB RAM.`);
